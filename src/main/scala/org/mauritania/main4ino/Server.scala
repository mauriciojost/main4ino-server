  ...
  def stream(args: List[String], requestShutdown: IO[Unit]): Stream[IO, ExitCode] = {
    for {
      ...
      cleanupRepoTask = for {
        ...
        cleaned <- repo.cleanup(Table.Reports, epSecs, 7 * DaySeconds)
        _ <- logger.info(s"Repository cleanup at $now ($epSecs): $cleaned cleaned")
      } yield (cleaned)

      _ <- Stream.eval(Database.initialize(transactor))
      _ <- Stream.eval(Scheduler.regularIO(cleanupRepoTask, FiniteDuration(DaySeconds, TimeUnit.SECONDS)))
