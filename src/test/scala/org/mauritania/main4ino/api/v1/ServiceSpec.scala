...
import org.mauritania.main4ino.models.{ActorTup, Device, RecordId, Status => S}
...
class ServiceSpec extends WordSpec with MockFactory with Matchers {
  ...
  "The service" should {
    ...
    "return 200 with an existent target/request when reading target/report requests" in {
      (r.selectDeviceWhereRequestId _).when(Table.Targets, 1L).returns(IO.pure(Some(Dev1))).once // mock
      (r.selectDeviceWhereRequestId _).when(Table.Reports, 1L).returns(IO.pure(Some(Dev1))).once() // mock
  ...
  it should {
    ...
    "return 200 with a list of existent targets when reading all targets request" in {
      (r.selectDevicesWhereTimestamp _).when(Table.Targets, "dev1", None, None, None).returns(Stream.fromIterator[IO, Device](Iterator(Dev1, Dev2))).once // mock
